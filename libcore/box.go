...
type BoxInstance struct {
	...
	*box.Box
	...
	// Mihomo support
	isMihomo          bool
	selectOutboundBox func(tag string) bool
	queryStatsBox     func(tag, direct string) int64
	urlTestBox        func(link string, timeout int32) (int32, error)
	testProxies       map[string]mihomoconst.Proxy
...
func newSingBoxInstance(config string, localTransport LocalDNSTransport, platformLog bool) (b *BoxInstance, err error) {
	...
	if strings.Contains(trimmed, `"core": "xray"`) ||
		strings.Contains(trimmed, `"core":"xray"`) ||
		strings.Contains(trimmed, `"xray_version"`) ||
		(strings.Contains(trimmed, `"protocol": "vless"`) && strings.Contains(trimmed, `"realitySettings"`)) ||
		(strings.Contains(trimmed, `"protocol": "freedom"`) && strings.Contains(trimmed, `"outbounds"`)) {
		return newXrayInstance(config, localTransport, platformLog)
	}
	if !strings.HasPrefix(trimmed, "{") ||
		...
		strings.Contains(trimmed, "\r\nproxies:") {
		return newMihomoInstance(config, localTransport, platformLog)
	}
...
func (b *BoxInstance) QueryStats(tag, direct string) int64 {
	if b.queryStatsBox != nil {
		return b.queryStatsBox(tag, direct)
	}
	if b.v2api == nil {
		return 0
	}
...
func (b *BoxInstance) SelectOutbound(tag string) bool {
	if b.selectOutboundBox != nil {
		return b.selectOutboundBox(tag)
	}
	if b.selector != nil {
...
func UrlTest(i *BoxInstance, link string, timeout int32) (latency int32, err error) {
	...
	if i != nil && i.urlTestBox != nil {
		return i.urlTestBox(link, timeout)
	}
