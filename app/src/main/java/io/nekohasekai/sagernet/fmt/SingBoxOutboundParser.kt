...
fun parseSingBoxOutbound(json: JSONObject): AbstractBean? {
    ...
    var server = json.getStr("server")
    var port = json.getIntNya("server_port")
    if (server.isNullOrBlank() && type == "wireguard") {
        val peers = json.optJSONArray("peers")
        if (peers != null && peers.length() > 0) {
            val firstPeer = peers.optJSONObject(0)
            if (firstPeer != null) {
                server = firstPeer.getStr("server")
                port = firstPeer.getIntNya("server_port") ?: firstPeer.getIntNya("port")
            }
        }
    }
...
private fun StandardV2RayBean.applySingBoxTLS(tls: SingBoxTLSFields?) {
    if (tls == null) return
    security = if (tls.realityPubKey.isNotBlank()) "reality" else "tls"
    sni = tls.sni
    alpn = tls.alpn
    allowInsecure = tls.allowInsecure
    certificates = tls.certificates
...
private fun StandardV2RayBean.applySingBoxTransport(json: JSONObject) {
    ...
    when (transport.getStr("type")) {
        ...
        "xhttp", "splithttp" -> {
            ...
            xhttpMode = normalizeXhttpMode(transport.getStr("mode"))
            transport.getStr("extra")?.let { xhttpExtra = it }
        }
    }
}
